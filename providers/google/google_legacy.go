...
func newConfig(provider *Provider, scopes []string) *oauth2.Config {
	c := &oauth2.Config{
		...
		// We can't use the `golang.org/x/oauth2/google` package directly on
		// old versions of Go, as it relies on the type alias syntax, so to
		// get around that, we copy the oauth2.Enpoint here from that package.
